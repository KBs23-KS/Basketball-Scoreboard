...
export default function Scoreboard() {
      ...
      if (isClockRunning) {
        setClockMs((prev) => {
          ...
          if (next <= 0) {
            ...
            if (settings.soundEnabled && !isDisplayMode)
      ...
      if (isShotClockRunning) {
        setShotClockMs((prev) => {
          ...
          if (next <= 0) {
            setIsShotClockRunning(false);
            if (settings.soundEnabled && !isDisplayMode)
              playHorn("shotclock", settings.soundVolume / 100);
            return 0;
          }
          return next;
        });
      ...
      if (activeTimeout) {
        setTimeoutMs((prev) => {
          ...
          if (next <= 0) {
            ...
            if (settings.soundEnabled && !isDisplayMode)
              playHorn("timeout", settings.soundVolume / 100);
            return 0;
          }
          // Warning beep at 10 seconds
          if (
            Math.floor(prev / 1000) === 10 &&
            Math.floor(next / 1000) === 9 &&
  ...
  const addLog = (message) => {
    ...
    setLogs((prev) =>
      [{ time: timeStr, msg: message, id: Date.now() }, ...prev].slice(0, 50)
    );
  };
  ...
  const updateScore = (team, points, playerId = null) => {
    ...
    if (isHome) {
      setBlinkHome(true);
      setTimeout(() => setBlinkHome(false), 500);
    } else {
      setBlinkAway(true);
      setTimeout(() => setBlinkAway(false), 500);
    }
    ...
    if (playerId) {
      ...
      if (pIdx >= 0) {
        ...
        logMsg = `${newPlayers[pIdx].name} (${teamData.name}) ${
          points > 0 ? "+" : ""
        }${points} คะแนน`;
      }
    }
  ...
  const updateFouls = (team, amount, playerId = null) => {
    ...
    if (playerId) {
      ...
      if (pIdx >= 0) {
        ...
        logMsg = `${newPlayers[pIdx].name} (${teamData.name}) ฟาวล์ ${
          amount > 0 ? "+1" : "-1"
        }`;
      }
    }
  ...
  const nextQuarter = () => {
    if (
      window.confirm("ยืนยันการเปลี่ยนควอเตอร์? (ระบบจะรีเซ็ตเวลาและฟาวล์ทีม)")
    ) {
  ...
  const renderFoulDots = (fouls, isBonus) => {
    ...
    return (
      <div className="flex flex-col items-center mt-2">
        ...
        <div
          className={`mt-1 font-bold text-xl tracking-widest ${
            isBonus ? "text-yellow-400 animate-pulse" : "text-gray-800"
          }`}
        >
          BONUS
        </div>
  ...
  const ScoreboardDisplay = ({ minimal = false }) => {
    ...
    const shotClockColor =
      shotClockMs <= 5000 ? "text-red-500" : "text-yellow-400";

    return (
      <div
        className="flex flex-col w-full h-full bg-black text-white font-sans select-none border-4 border-gray-800 overflow-hidden"
      ...
      >
        ...
        <div className="flex flex-1 flex-col md:flex-row items-stretch justify-between p-2 md:p-6 gap-4">
          {/* HOME TEAM */}
          <div
            className="flex-1 flex flex-col items-center justify-around bg-gray-950 p-4 rounded-2xl border-2 shadow-2xl relative"
            style={{ borderColor: home.color }}
          >
            <div
              className={`absolute inset-0 bg-blue-500 opacity-0 transition-opacity duration-300 pointer-events-none rounded-xl ${
                blinkHome ? "opacity-20" : ""
              }`}
            ></div>

            <div className="flex items-center gap-4">
              <h2
                className="text-4xl md:text-6xl font-black uppercase truncate max-w-[300px]"
            ...
            <div
              className="text-[8rem] md:text-[14rem] font-mono leading-none font-bold text-white tracking-tighter"
            ...
            <div className="flex w-full justify-around mt-4">
              <div className="flex flex-col items-center">
                <span className="text-gray-400 uppercase font-bold text-xl">
                  FOULS
                </span>
                <span className="text-5xl font-mono text-red-500 font-bold">
                  {home.fouls}
                </span>
                  ...
                  {[...Array(settings.timeoutsPerTeam)].map((_, i) => (
                    <div
                      ...
                      className={`w-8 h-3 rounded-sm ${
                        i < home.timeouts ? "bg-yellow-400" : "bg-gray-800"
                      }`}
                    ></div>
                  ))}
                </div>
          ...
          {/* CENTER - CLOCKS */}
          <div className="flex-[0.8] flex flex-col items-center justify-center bg-gray-900 p-4 rounded-2xl border border-gray-700 shadow-xl z-10 relative">
            {activeTimeout ? (
              <div className="flex flex-col items-center mb-4 w-full bg-yellow-600 rounded-lg p-4 animate-pulse">
                <span className="text-2xl font-bold uppercase">
                  TIMEOUT - {activeTimeout === "home" ? home.name : away.name}
                </span>
            ...
            <div
              className={`text-[6rem] md:text-[9rem] font-mono font-bold leading-none tracking-tight ${clockColor} ${
                !isClockRunning && clockMs > 0 ? "opacity-80" : ""
              }`}
              style={{ fontVariantNumeric: "tabular-nums" }}
            >
              {formatTime(clockMs)}
            </div>
            {clockMs === 0 && !isClockRunning && (
              <div className="text-red-500 font-bold text-2xl mt-[-10px] mb-4 animate-pulse">
                END OF {quarter}
              </div>
            )}

            <div className="mt-8 flex flex-col items-center bg-black p-6 rounded-xl border border-gray-800">
              <span className="text-gray-500 text-sm font-bold uppercase tracking-widest mb-2">
                SHOT CLOCK
              </span>
              <div
                className={`text-[5rem] md:text-[7rem] font-mono font-bold leading-none ${shotClockColor}`}
              >
                {shotClockMs === 0
                  ? "00"
                  : formatShotClock(shotClockMs).padStart(2, "0")}
              </div>
            </div>
          </div>
          ...
          >
            <div
              className={`absolute inset-0 bg-red-500 opacity-0 transition-opacity duration-300 pointer-events-none rounded-xl ${
                blinkAway ? "opacity-20" : ""
              }`}
            ></div>
                ...
                <div className="flex gap-2 mt-2">
                  {[...Array(settings.timeoutsPerTeam)].map((_, i) => (
                    <div
                      key={i}
                      className={`w-8 h-3 rounded-sm ${
                        i < away.timeouts ? "bg-yellow-400" : "bg-gray-800"
                      }`}
                    ></div>
                  ))}
                </div>
  ...
  if (isDisplayMode) {
    return (
      <div className="w-screen h-screen bg-black overflow-hidden flex items-center justify-center p-4">
        <ScoreboardDisplay />
      </div>
    );
  }
  ...
  const ControlButton = ({
    ...
    <button
      onClick={onClick}
      disabled={disabled}
      className={`flex flex-col items-center justify-center p-3 rounded-lg shadow-md transition-all active:scale-95 ${color} ${
  ...
  const TeamControls = ({ team, type }) => {
    ...
    return (
      <div
        className="bg-gray-800 p-4 rounded-xl space-y-4 border-t-4"
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            <ControlButton
              ...
              onClick={() => updateScore(type, 1)}
            />
            <ControlButton
              label="+2"
              color="bg-green-600 hover:bg-green-500"
              onClick={() => updateScore(type, 2)}
            />
            <ControlButton
              label="+3"
              color="bg-green-600 hover:bg-green-500"
              onClick={() => updateScore(type, 3)}
            />
            <ControlButton
              label="-1"
              ...
              <ControlButton
                ...
                onClick={() => {
                  ...
                  if (val !== null && !isNaN(val)) {
                    saveHistorySnapshot();
                    type === "home"
                      ? setHome({ ...home, score: parseInt(val) })
                      : setAway({ ...away, score: parseInt(val) });
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="color"
                value={home.color}
                onChange={(e) => setHome({ ...home, color: e.target.value })}
            ...
            <div>
              ...
              <input
                type="text"
                value={away.name}
                onChange={(e) => setAway({ ...away, name: e.target.value })}
                className="w-full bg-gray-700 p-2 rounded"
              />
            </div>
            <div>
              ...
              <input
                type="color"
                value={away.color}
                onChange={(e) => setAway({ ...away, color: e.target.value })}
                className="w-full h-10 bg-gray-700 p-1 rounded"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm mb-1">
                เวลาต่อควอเตอร์ (นาที)
              </label>
              <input
                type="number"
                value={settings.quarterMinutes}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    quarterMinutes: parseInt(e.target.value) || 10,
                  })
                }
                className="w-full bg-gray-700 p-2 rounded"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    timeoutDuration: parseInt(e.target.value) || 60,
                  })
                }
                className="w-full bg-gray-700 p-2 rounded"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    shotClockSeconds: parseInt(e.target.value) || 24,
                  })
                }
                className="w-full bg-gray-700 p-2 rounded"
              />
            ...
            <div>
              ...
              <input
                type="number"
                value={settings.bonusFoulLimit}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    bonusFoulLimit: parseInt(e.target.value) || 5,
                  })
                }
                className="w-full bg-gray-700 p-2 rounded"
              />
            ...
            <div>
              <label className="block text-sm mb-1">
                ฟาวล์ผู้เล่น (Foul Out)
              </label>
              <input
                type="number"
                value={settings.playerFoulOutLimit}
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    playerFoulOutLimit: parseInt(e.target.value) || 5,
                  })
                }
                className="w-full bg-gray-700 p-2 rounded"
              />
  ...
  const RosterPanel = () => {
    const renderTeamRoster = (team, setTeamObj) => (
      <div
        className="flex-1 bg-gray-800 p-4 rounded-xl border-t-4"
              ...
              {team.players.map((p) => (
                <tr
                  ...
                  className={`border-b border-gray-700 ${
                    p.fouls >= settings.playerFoulOutLimit
                      ? "bg-red-900/50"
                      : ""
                  }`}
                >
                  ...
                  <td className="px-2 py-2">
                    {p.name}{" "}
                    {p.fouls >= settings.playerFoulOutLimit && (
                      <span className="text-xs bg-red-600 text-white px-1 rounded ml-1">
                        FOUL OUT
                      </span>
                    )}
                  </td>
                  <td className="px-2 py-2 font-mono text-lg">{p.points}</td>
                  <td className="px-2 py-2 font-mono text-lg text-red-400">
                    {p.fouls}
                  </td>
                  <td className="px-2 py-2 flex gap-1">
                    <button
                      onClick={() =>
                        updateScore(team === home ? "home" : "away", 1, p.id)
                      }
                    ...
                    <button
                      onClick={() =>
                        updateScore(team === home ? "home" : "away", 2, p.id)
                      }
                      className="px-2 py-1 bg-green-700 rounded hover:bg-green-600 text-xs"
                    >
                      +2P
                    ...
                    <button
                      onClick={() =>
                        updateScore(team === home ? "home" : "away", 3, p.id)
                      }
                      className="px-2 py-1 bg-green-700 rounded hover:bg-green-600 text-xs"
                    >
                      +3P
                    ...
                    <button
                      onClick={() =>
                        updateFouls(team === home ? "home" : "away", 1, p.id)
                      }
                      className="px-2 py-1 bg-yellow-700 rounded hover:bg-yellow-600 text-xs"
                      disabled={p.fouls >= settings.playerFoulOutLimit}
                    >
                      +F
                    </button>
                  </td>
      ...
      <div className="bg-white text-black p-8 rounded max-w-4xl mx-auto shadow-lg printable-area">
        ...
        <div className="text-center border-b-2 border-gray-800 pb-4 mb-6">
          ...
          <p className="text-gray-600">
            สรุปผลการแข่งขัน • {new Date().toLocaleDateString("th-TH")}
          </p>
        </div>

        <div className="flex justify-between items-center mb-8 bg-gray-100 p-6 rounded-xl">
          <div className="text-center w-1/3">
            <h2 className="text-2xl font-bold" style={{ color: home.color }}>
              {home.name}
            </h2>
          ...
          <div className="text-center w-1/3">
            <h2 className="text-2xl font-bold" style={{ color: away.color }}>
              {away.name}
            </h2>
        ...
        <div className="flex justify-center gap-4 mt-8 no-print">
          <button
            onClick={handlePrint}
            className="px-6 py-2 bg-blue-600 text-white rounded shadow hover:bg-blue-700 flex items-center gap-2"
          >
      ...
      <nav className="bg-gray-900 p-2 border-b border-gray-800 flex justify-between items-center px-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 bg-green-700 hover:bg-green-600 rounded text-sm font-bold text-white shadow"
          >
          ...
          <div className="flex flex-col lg:flex-row gap-4 h-full">
            ...
            <div className="w-full lg:w-1/3 flex flex-col gap-4">
              <div className="h-64 rounded-xl overflow-hidden border border-gray-700 relative">
                <div className="absolute top-1 left-2 text-[10px] text-gray-400 font-mono z-20">
                  PREVIEW
                </div>
              ...
              <div className="bg-gray-900 rounded-xl p-4 flex-1 border border-gray-800 flex flex-col">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="font-bold text-gray-300 text-sm uppercase">
                    ประวัติการทำรายการ
                  </h3>
                  <div className="flex gap-2">
                    <button
            ...
            <div className="w-full lg:w-2/3 flex flex-col gap-4">
              ...
              <div className="bg-gray-900 p-4 rounded-xl border border-gray-800 flex flex-wrap gap-4 items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={toggleClock}
                    className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold text-white shadow-lg ${
                      isClockRunning
                  ...
                  >
                    {isClockRunning ? (
                      <>
                        <Square size={20} /> หยุดเวลา (Space)
                      </>
                    ) : (
                      <>
                        <Play size={20} /> เริ่มเวลา (Space)
                      </>
                    )}
                  </button>
                  ...
                  <div className="flex flex-col gap-1 ml-2">
                    <button
                      onClick={() => setClockMs((m) => m + 60000)}
                      className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs"
                    >
                      +1 นาที
                    </button>
                    <button
                      onClick={() => setClockMs((m) => m + 1000)}
                      className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs"
                    >
                      +1 วิ
                    </button>
                  ...
                  <div className="flex flex-col gap-1">
                    <button
                      onClick={() => setClockMs((m) => Math.max(0, m - 60000))}
                      className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs"
                    >
                      -1 นาที
                    </button>
                    <button
                      onClick={() => setClockMs((m) => Math.max(0, m - 1000))}
                      className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs"
                    >
                      -1 วิ
                    </button>
                  ...
                  <button
                    onClick={resetClock}
                    className="p-3 bg-gray-700 hover:bg-gray-600 rounded-lg ml-2"
                    title="รีเซ็ตเวลา (Shift+R)"
                  >
                    <RotateCcw size={20} />
                  </button>
                </div>
                ...
                <div className="flex items-center gap-2 bg-gray-800 p-2 rounded-lg">
                  ...
                  <button
                    ...
                    className={`p-2 rounded ${
                      isShotClockRunning ? "bg-red-800" : "bg-green-800"
                    }`}
                  >
                    {isShotClockRunning ? (
                      <Square size={16} />
                  ...
                  <button
                    onClick={() => resetShotClock(settings.shotClockSeconds)}
                    className="px-3 py-2 bg-blue-900 hover:bg-blue-800 font-mono font-bold rounded text-sm"
                  >
                    {settings.shotClockSeconds}
                  </button>
                  <button
                    onClick={() =>
                      resetShotClock(settings.shortShotClockSeconds)
                    }
                    className="px-3 py-2 bg-blue-900 hover:bg-blue-800 font-mono font-bold rounded text-sm"
                  >
                    {settings.shortShotClockSeconds}
                  </button>
                  <button
                    onClick={() => setShotClockMs((m) => m + 1000)}
                    className="px-2 py-2 bg-gray-700 rounded text-xs"
                  >
                    +1
                  </button>
                  <button
                    onClick={() => setShotClockMs((m) => Math.max(0, m - 1000))}
                    className="px-2 py-2 bg-gray-700 rounded text-xs"
                  >
                    -1
                  </button>
                ...
                <div className="bg-gray-900 p-3 rounded-xl border border-gray-800 flex flex-1 items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="p-2 bg-orange-700 hover:bg-orange-600 rounded-lg flex items-center gap-1 text-sm font-bold shadow-lg"
                    >
                    ...
                    {activeTimeout && (
                      <button
                        onClick={stopTimeout}
                        className="p-2 bg-red-700 hover:bg-red-600 rounded-lg text-sm font-bold shadow-lg"
                      >
                        จบเวลานอก
                      </button>
                    )}
